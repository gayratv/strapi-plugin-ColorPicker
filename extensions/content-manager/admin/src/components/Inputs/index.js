...
function Inputs({
  ...
  const fieldName = useMemo(() => {
    const rets = getFieldName(keys);
    // console.log("fieldName ", rets, keys);
  ...
  const inputType = useMemo(() => {
    const rets = getInputType(type);
    // console.log("inputType : ", rets);
    return rets;
  }, [type]);
  ...
  const otherFields = useMemo(() => {
    const flds = fieldApi.getFields();
    // console.log("Inputs otherFields : ", flds);
    return flds;
  }, [fieldApi]);
  ...
  const editViewDataManagerContext = useContext(EditViewDataManagerContext);
  EditViewDataManagerContext.Provider.console.log(
